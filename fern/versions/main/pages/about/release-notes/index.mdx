...
- **`InferenceServer` and typed backend configuration**: APIs to deploy one or more LLMs as OpenAI-compatible endpoints directly within a Ray cluster. Use `RayServeModelConfig` for Ray Serve or `DynamoVLLMModelConfig` with `DynamoServerConfig` for NVIDIA Dynamo.
